...
def parse_de(data, version, save, skip=False):
    ...
    for _ in range(num_players if 66.3 > save >= 37 else 8):
        ...
        if save >= 61.5:
            ...
            # NOTE: as of save_version 68.0 (confirmed against 3 replays, all
            # save 68.0 -- no data points between 63.0 and 68.0 to pin the
            # exact cutoff, so this may need adjusting once one surfaces),
            # the civ pool is no longer serialized as `custom_civ_count`
            # trailing civ ids (4 bytes each). Byte-level inspection shows:
            #   - custom_civ_count == 0  -> 0 trailing bytes (unchanged)
            #   - custom_civ_count > 0   -> exactly 4 trailing bytes total,
            #     regardless of the count value (a single field -- possibly
            #     a hash/bitmask of the pool -- replaces the per-id array).
            # Reading the old N*4-byte array here consumes real header bytes
            # (including the next field's de_string marker), corrupting
            # every field read afterwards for that player slot (name,
            # profile_id, handicap, ...) and eventually raising
            # AssertionError in de_string() for a later slot or player.
            # See docs/PIPELINE.md "Parsing bugs fixed".
            if 68.0 > save >= 63.0 and custom_civ_count > 0:
                custom_civ_selection = []
                for _ in range(custom_civ_count):
                    custom_civ_selection.append(unpack('<I', data))
            elif save >= 68.0 and custom_civ_count > 0:
                custom_civ_selection = unpack('<I', data)
        de_string(data)
        data.read(1)
    ...
    data.read(4)
    if save >= 68.0:
        # NOTE: an extra 4 bytes appear here as of save_version 68.0 (same
        # caveat as the custom-civ-array change above: unconfirmed exact
        # cutoff, no data points between 63.0 and 68.0). Content is 0 across
        # 3 distinct replays (not game-specific data, so likely a new
        # reserved/flag field rather than variable-length data) -- verified
        # by cross-checking rated/allow_specs/spec_delay come out as sane
        # values (1/1/120 vs. garbage without this skip) and that
        # string_block's leading crc + de_string marker land exactly where
        # expected afterwards. See docs/PIPELINE.md "Parsing bugs fixed".
        data.read(4)
    rated = unpack('b', data)

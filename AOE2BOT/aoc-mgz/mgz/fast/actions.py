...
def parse_action_71094(action_type, player_id, raw):
    ...
    if action_type is Action.DE_QUEUE:
        # NOTE: the upstream format string ('<h4xhhh4x') ends with a spurious
        # trailing 4-byte skip that consumes the very bytes meant to hold the
        # object_ids array, so the unpack below always failed with
        # "unpack requires a buffer of 4 bytes" and every Queue action
        # silently degraded to Action.ERROR. Confirmed against known-good
        # values (unit_id=83 "Villager", object_ids=[2266] "Town Center")
        # recovered from an older aoc-mgz build's JSON output for the same
        # replay -- see docs/PIPELINE.md "Parsing bugs fixed".
